...
public class HeraldGunAttackGoal<T extends PathfinderMob> extends Goal {
    ...
    public HeraldGunAttackGoal(T shooter, ItemStack gunStack, float speedModifier, AIType aiType, int difficulty) {
            ...
            if (heldItem.getTag().getInt("AmmoCount") <= 0) {
                if (!this.isReloading) {
                    if (this.shooter instanceof AbstractGravekeeperGunnerEntity animatable) {
                        this.shooter.hurt(this.shooter.damageSources().mobAttack(this.shooter), 4.0f);
                    }
                    ...
                    this.reloadTick = 10; //gun.getReloads().getReloadTimer()+40;
